...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!userBusiness) return NextResponse.json({ error: "Bu işletmeye erişiminiz yok" }, { status: 403 });

    await prisma.userBusiness.updateMany({ where: { userId: session.user.id }, data: { isDefault: false } });
